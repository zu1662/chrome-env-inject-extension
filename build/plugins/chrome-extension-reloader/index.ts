...
interface ChromeExtensionReloaderOptions {
   ...
   */
  debugPages?: ('popup' | 'background' | 'options' | 'sandbox' | 'devtools')[];
  entry: {
    background: string;
    popup?: string;
    options?: string;
...
export class ChromeExtensionReloaderWebpackPlugin {
  ...
  setupWebpackConfig(compiler: Compiler) {
    ...
    const sandboxEntry = entry.sandbox
      ? this.getNormalEntry('sandbox', entry.sandbox, { addDevServerScripts: isDev })
      : {};
    const devtoolsEntry = entry.devtools
      ? this.getNormalEntry('devtools', entry.devtools, { addDevServerScripts: isDev })
      : {};
      ...
      splitChunks: {
        cacheGroups: {
          vendor: {
            name: 'vendor',
            chunks(chunk) {
              return ['popup', 'options', 'sandbox'].includes(chunk.name || '');
            },
            test: /[\\/]node_modules[\\/]/,
            priority: -10
          },
